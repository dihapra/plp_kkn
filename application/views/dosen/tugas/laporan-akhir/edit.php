...
<div class="container mt-3">
    <h2 class="mb-3">Edit Penilaian Laporan Kemajuan</h2>
    ...
    <!-- Step Indicator (opsional, tetap 2 langkah) -->
    <div class="d-flex justify-content-center mb-3 gap-4">
        <div id="circleStep1" class="step-circle">
            <div class="circle">1</div>
            <div class="label">Revisi</div>
        </div>
        <div id="circleStep2" class="step-circle active">
            <div class="circle">2</div>
            <div class="label">Penilaian</div>
        </div>
    </div>

    <!-- Langsung tampilkan form penilaian (edit) -->
    <form id="penilaianForm" class="mb-5">
                        ...
                        foreach ($aspek_laporan as $aspek): ?>
                            <tr>
                                ...
                                <?php foreach ($anggota_ids as $sid):
                                    ...
                                    $existing = $scoreLapMap[$aspek['key']][$sidStr] ?? null;
                                ?>
                                    <td>
                                        <select class="form-select nilai-laporan"
                                            name="nilai_laporan[<?= htmlspecialchars($aspek['key']) ?>][<?= (int)$sid ?>]"
                    ...
                    <tbody>
                        ...
                        foreach ($aspek_presentasi as $aspek): ?>
                            <tr>
                                ...
                                <?php foreach ($anggota_ids as $sid):
                                    $sidStr = (string)(int)$sid;
                                    $existing = $scorePresMap[$aspek['key']][$sidStr] ?? null;
                                ?>
                                    <td>
                                        <select class="form-select nilai-presentasi"
                                            name="nilai_presentasi[<?= htmlspecialchars($aspek['key']) ?>][<?= (int)$sid ?>]"
                                            data-student-id="<?= (int)$sid ?>">
                                            <?php for ($i = 1; $i <= 4; $i++): ?>
                                                <option value="<?= $i ?>" <?= ($existing === $i ? 'selected' : '') ?>><?= $i ?></option>
                                            <?php endfor; ?>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" name="feedback" id="feedback" rows="3" placeholder="Masukkan komentar atau feedback…"><?= htmlspecialchars($detail_laporan->feedback ?? '') ?></textarea>
        </div>

        <div class="sticky-actions">
            <button type="submit" class="btn btn-primary" id="btnSimpanPenilaian">Simpan Perubahan</button>
        </div>
    </form>
</div>

<script>
    // Recalc total & nilai (persentase) per mahasiswa
    (function() {
        const aspekLaporanCount = <?= (int)count($aspek_laporan) ?>;
        const aspekPresCount = <?= (int)count($aspek_presentasi) ?>;

        function recalc(blockClass, totalPrefix, nilaiPrefix, aspekCount) {
            const byStudent = {};
            document.querySelectorAll('.' + blockClass).forEach(sel => {
                const sid = sel.getAttribute('data-student-id');
                const v = parseInt(sel.value || '0', 10) || 0;
                byStudent[sid] = (byStudent[sid] || 0) + v;
            });
            Object.keys(byStudent).forEach(sid => {
                const total = byStudent[sid] || 0;
                const persen = aspekCount > 0 ? Math.round((total / (aspekCount * 4)) * 100) : 0;
                const tEl = document.getElementById(totalPrefix + sid);
                const nEl = document.getElementById(nilaiPrefix + sid);
                if (tEl) tEl.textContent = total;
                if (nEl) nEl.textContent = persen;
            });
        }

        function recalcAll() {
            recalc('nilai-laporan', 'total-laporan-', 'nilai-laporan-', aspekLaporanCount);
            recalc('nilai-presentasi', 'total-presentasi-', 'nilai-presentasi-', aspekPresCount);
        }

        // hitung awal & saat berubah
        document.addEventListener('DOMContentLoaded', recalcAll);
        document.addEventListener('change', function(e) {
            if (e.target.matches('.nilai-laporan, .nilai-presentasi')) recalcAll();
        });
    })();
</script>
<script>
        ...
        btnRevisi?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    location.href = "<?= base_url('dosen/tugas/laporan-akhir') ?>";
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
