...
<div class="container mt-3">
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h2 class="mb-0">Penilaian Bahan Ajar</h2>
        <a href="<?= base_url('dosen/tugas/bahan-ajar') ?>" class="btn btn-outline-secondary">← Kembali</a>
    </div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                Swal.fire({
                    ...
                    window.location.href = "<?= base_url('dosen/tugas/bahan-ajar') ?>";
                });
            });
        </script>
    <?php endif; ?>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        btnRevisi?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    location.href = "<?= base_url('dosen/tugas/bahan-ajar') ?>";
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Gagal',
        ...
        document.getElementById('penilaianForm')?.addEventListener('submit', async (e) => {
            ...
            try {
                const resp = await fetch("<?= base_url('dosen/ajar/save/4') ?>", {
                    method: 'POST',
                    body: fd
                });
