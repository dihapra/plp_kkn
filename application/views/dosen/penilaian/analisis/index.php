...
$endpointMap = [
    'analisis' => [
        'view'  => 'dosen/penilaian/analisis/view/{student_id}',
        'score' => 'dosen/penilaian/analisis/{student_id}',
        'edit'  => 'dosen/penilaian/analisis/edit/{student_id}',
    ],
];
?>
...
<div class="card">
    <div class="card-body">
        <h4 class="mb-3"><?= $title ?? 'Penilaian Analisis Mahasiswa' ?></h4>
                ...
                <tbody>
                    <?php if (!empty($students)):
                        foreach ($students as $row):
                            ...
                            $has_analisis = is_object($row) ? (int)($row->has_analisis ?? 0) : (int)($row['has_analisis'] ?? 0);
                    ?>
                            <tr>
                                <td><?= htmlentities($sname) ?></td>
                                <td>
                                    <?= render_evaluation_status($has_analisis, 'Penilaian Analisis Mahasiswa') ?>
                                </td>
                                <td>
                                    <?= render_evaluation_actions($has_analisis, $sid, 'Penilaian Analisis Mahasiswa', 'analisis', $endpointMap) ?>
                                </td>
                            </tr>
                        <?php endforeach;
                    else: ?>
