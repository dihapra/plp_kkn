...
<div class="container mt-3">
    <div class="d-flex justify-content-between align-items-center mb-2">
        <h2 class="mb-0">Penilaian Analisis Mahasiswa</h2>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Simpan -> type=4 (analisis)
        document.getElementById('penilaianForm')?.addEventListener('submit', async (e) => {
            e.preventDefault();
            const fd = new FormData(e.target);
            try {
                const resp = await fetch("<?= base_url('dosen/nilai-intra-ekstra-sikap/save/4') ?>", {
                    method: 'POST',
                    body: fd
                });
                ...
                if (resp.ok) {
                    if (window.Swal) {
                        ...
                        location.href = "<?= base_url('dosen/penilaian/analisis') ?>";
                    } else {
