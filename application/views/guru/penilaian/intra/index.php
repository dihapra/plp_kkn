...
$endpointMap = [
    'intrakurikuler' => [
        'view'  => 'guru/intra_view/{student_id}',
        'score' => 'guru/intra/{student_id}',
        'edit'  => 'guru/intra_edit/{student_id}',
    ],
];
?>
...
<div class="card mt-4">
    <div class="card-header">
        <div class="card-title">Daftar Mahasiswa untuk Penilaian Intrakurikuler</div>
    </div>
                ...
                <tbody>
                    <?php if (!empty($students)):
                        foreach ($students as $row):
                            ...
                            $has_intra = is_object($row) ? (int)($row->has_intracurricular ?? 0) : (int)($row['has_intracurricular'] ?? 0);
                    ?>
                            <tr>
                                ...
                                <!-- Intrakurikuler -->
                                <td>
                                    <?= render_evaluation_status($has_intra, 'Penilaian Intrakurikuler') ?>
                                </td>
                                <td>
                                    <?= render_evaluation_actions($has_intra, $sid, 'Penilaian Intrakurikuler', 'intrakurikuler', $endpointMap) ?>
                                </td>
                            </tr>
                        <?php endforeach;
                    else: ?>
