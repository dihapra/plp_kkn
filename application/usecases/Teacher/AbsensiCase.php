...
class AbsensiCase
{
    ...
    public function verif_by_teacher()
    {
        try {
            ...
            // Meeting Restrictions: Teachers will fill attendance for meetings *other than* [1, 2, 3, 8, 15, 16].
            $restricted_meetings = [1, 2, 3, 8, 15, 16];
            if (in_array($meeting_number, $restricted_meetings)) {
                throw new Exception('Pertemuan ini hanya bisa diisi oleh Dosen.');
            }
            ...
            $data_to_input = [
                ...
                'verify_by_teacher' => $teacher_id, // Corrected from verify_by_lecture
                'status' => $status,
                'date' => date('Y-m-d'),
