...
class GuruCase extends BaseCase
{
    public function create(array $payload, array $files = []): void
    {
        ...
        try {
            ...
            if (empty($email)) {
                throw new \InvalidArgumentException('Email wajib diisi untuk akun guru.');
            }
            ...
            if ($userId <= 0) {
                throw new \RuntimeException('Gagal membuat user untuk guru.');
            }

            $data['id_user']    = $userId;
            $data['created_at'] = date('Y-m-d H:i:s');

            $id = $this->GuruRepository->create($data);

            $paths = $this->handleUploads($id, $files);
            if (!empty($paths)) {
                $this->GuruRepository->update($id, $paths);
            }
    ...
    public function update(int $id, array $payload, array $files = []): void
    {
        ...
        try {
            $row = $this->GuruRepository->find($id);
            if (!$row) {
            ...
            $this->GuruRepository->update($id, $updateData);
    ...
    {
        $result = $this->GuruRepository->datatable($params);
    ...
    private function handleUploads(int $id, array $files, $existing = null): array
    {
        ...
        $encryptedId = md5($id);
        $basePath    = FCPATH . 'uploads/guru/' . $encryptedId . '/';
        ...
        if (isset($files['foto_ktp']) && is_uploaded_file($files['foto_ktp']['tmp_name'])) {
            ...
            if (move_uploaded_file($files['foto_ktp']['tmp_name'], $target)) {
                $result['foto_ktp'] = 'uploads/guru/' . $encryptedId . '/' . $filename;
            }
        }
        ...
        if (isset($files['buku']) && is_uploaded_file($files['buku']['tmp_name'])) {
            ...
            if (move_uploaded_file($files['buku']['tmp_name'], $target)) {
                $result['buku'] = 'uploads/guru/' . $encryptedId . '/' . $filename;
            }
        }
